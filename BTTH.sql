...
BEGIN
    ...
    IF v_price IS NULL THEN
        SET p_message = 'That bai: Thuoc khong ton tai';

    --  nếu số lượng không hợp lệ 
    ELSEIF p_quantity <= 0 THEN
        SET p_message = 'That bai: So luong khong hop le';
...
--    TEST 1: bình thường (không giảm giá)
--    medicine 1 còn nhiều (100)
...
--    TEST 2: có mã NV-RIKKEI (giảm 50%)

CALL ProcessPrescription(1, 1, 2, 'NV-RIKKEI', @msg);
SELECT @msg;


--    TEST 3: vượt tồn kho (medicine 2 chỉ có 5)

CALL ProcessPrescription(1, 2, 10, NULL, @msg);
SELECT @msg;

-- xem tồn kho sau khi test
SELECT * FROM Medicines;

-- xem công nợ bệnh nhân
