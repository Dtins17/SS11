...
-- Tái hiện lỗi: Hủy một lịch khám đã hoàn tất
CALL CancelAppointment(105);

-- Kiểm tra kết quả: Trạng thái sẽ bị chuyển thành 'Cancelled' dù đã khám xong
SELECT appointment_id, status FROM Appointments WHERE appointment_id = 105;

-- Nguyên nhân gây lỗi
-- Thủ tục CancelAppointment hiện tại chỉ thực hiện lệnh UPDATE 
-- dựa trên appointment_id mà không kiểm tra trạng thái hiện tại của lịch khám. Do đó, bất kể lịch khám đang là 'Pending', 'Completed' hay đã 'Cancelled' trước đó, 
-- hệ thống vẫn ghi đè trạng thái mới, vi phạm quy tắc nghiệp vụ chỉ cho phép hủy lịch đang chờ ('Pending').

-- SỬA CHỮA MÃ NGUỒN 
-- Xóa thủ tục cũ 
DROP PROCEDURE IF EXISTS CancelAppointment;
-- Tạo thủ tục mới với logic chuẩn 
DELIMITER //
...
BEGIN
    ...
    -- Kiểm tra điều kiện: Chỉ xử lý nếu trạng thái là 'Pending'
    IF v_current_status = 'Pending' THEN
        UPDATE Appointments
        SET status = 'Cancelled'
        WHERE appointment_id = p_appointment_id;
        
        SELECT 'Success: Appointment has been cancelled.' AS Message;
    
    -- Nếu lịch không tồn tại
    ELSEIF v_current_status IS NULL THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Error: Appointment ID does not exist.';
    
    -- Nếu lịch đã hoàn tất hoặc đã hủy rồi
    ELSE
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Error: Only appointments with "Pending" status can be cancelled.';
    END IF;
END //
...
-- Kiểm tra trạng thái (IF v_current_status = 'Pending'): Đảm bảo tính toàn vẹn của luồng nghiệp vụ.

-- Sử dụng SIGNAL SQLSTATE '45000': Đây là cách chuyên nghiệp để trả về một thông báo lỗi từ Database. Nếu tiếp tân cố tình hủy lịch 'Completed', 
-- hệ thống sẽ chặn đứng hành động này và hiển thị thông báo lỗi cụ thể thay vì âm thầm cập nhật sai.

-- Tính an toàn: Tránh việc "chốt sổ" của kế toán bị sai lệch dữ liệu doanh thu từ các ca khám đã thực hiện xong. 